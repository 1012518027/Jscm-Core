package com.scm.x64.export;//package com..export;
...
public class MemoryOperationUtilsX64 {
    /**
     * 搜索的内容字节
     */
     ...
     * @return  返回结果 逻辑型
     */
    public static TagPmemory_Basic_InfoRmationX64 selectMemoryAddressAttribute(int hProcess, long address){
        TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = new TagPmemory_Basic_InfoRmationX64();
        ModuleOperationUtilsJNI.MyVirtualQueryEx(hProcess,address, pmemory_basic_information,64);
     ...
     * @param hProcess 进程句柄
     * @param address 内存地址 内存地址
     * @param pmemory_basic_information 内存属性 new PMEMORY_BASIC_INFORMATION()
     * @return  返回结果
     */
    public static int selectMemoryAddressAttribute(int hProcess, long address, TagPmemory_Basic_InfoRmationX64 pmemory_basic_information){
        int result = ModuleOperationUtilsJNI.MyVirtualQueryEx(hProcess,address, pmemory_basic_information,64);
        return result;
    }
     ...
     * #禁止访问 = 1
     */
    public static int updateMemoryAddressAttribute(int hProcess, long address,long dwSize,int attribute ){
        if(attribute==0){
            attribute = 64;
        }
        if(dwSize==0){
     ...
     * @return  返回结果
     */
    public static boolean memoryAddressIsValid(int hProcess,long address){
        boolean flag = false;
        TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = MemoryOperationUtilsX64.selectMemoryAddressAttribute(hProcess,address);
     ...
     * @return  返回结果
     */
    public static boolean memoryAddressIsStatic(int hProcess,long address){
        boolean flag = false;
        TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = MemoryOperationUtilsX64.selectMemoryAddressAttribute(hProcess,address);
     ...
     * @return  返回结果
     */
    public static boolean memoryAddressIsBaseAddress(int hProcess,long address){
        boolean flag = false;
        TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = MemoryOperationUtilsX64.selectMemoryAddressAttribute(hProcess,address);
     ...
     * @return  返回结果
     */
    public static long memoryAddressGetLength(int hProcess,long address){
        TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = MemoryOperationUtilsX64.selectMemoryAddressAttribute(hProcess,address);
        long len = pmemory_basic_information.RegionSize + pmemory_basic_information.BaseAddress - address;
        return len;
    }
     ...
     * @return  返回结果
     */
    public static byte[] readProcessMemoryByte(int hProcess, long address,long dwSize){
        byte[] buffer =new byte[(int) dwSize];
     ...
     * @return  返回结果
     */
    public static byte[] readProcessMemoryByteOffSet(int hProcess, long address,long dwSize,int[] offset){
     ...
     * @return  返回结果
     */
    public static int writeProcessMemoryByte(int hProcess,long address,int DwSize,byte[] lpBuffer){
     ...
     * @return  返回结果
     */
    public static int writeProcessMemoryByteOffSet(int hProcess,long address,int DwSize,byte[] lpBuffer,int[] offset){
        for (int i =0;i<offset.length;i++){
            byte[] offsetInt  = readProcessMemoryByte(hProcess,address,4);
            address = ByteUtils.bytesArrayToInt(offsetInt) + offset[i];
        }
     ...
     * @param model 0 采用sunday搜索  1采用传统方式搜索   2采用模糊搜索手动匹配自定义类型
    ...
    public static int threadSearchMemory(int hProcess, byte[] HexData,int model, MemoryCallBack memoryCallBack ){
        File file = new File(ModuleOperationUtilsJNI.tileDir()+ MemoryOperationUtilsX64.class.getSimpleName()+".ini");
        file.delete();
        ...
        final TagPmemory_Basic_InfoRmationX64 pmemory_basic_information = new TagPmemory_Basic_InfoRmationX64();
        while (selectMemoryAddressAttribute(hProcess,MemoryAddress,pmemory_basic_information) !=-1){
            if(pmemory_basic_information.State== 4096 && (pmemory_basic_information.lType == 16777216 ||  pmemory_basic_information.lType == 262144 || pmemory_basic_information.lType == 131072) && (pmemory_basic_information.Protect==64 || pmemory_basic_information.Protect==4)){
                ...
                if(MemoryAddress > 0){
                    byte[] data = MemoryOperationUtilsX64.readProcessMemoryByte(hProcess,MemoryAddress,pmemory_basic_information.RegionSize);
                    if(data.length > 0 ){
                        if (model==0){
                            long finalMemoryAddress = MemoryAddress;
    ...
    public static String readIniFile(String filePath,String hex){
        if(StringUtils.equals(filePath,null)){
            return IniConfigurationOperationUtils.readByte(ModuleOperationUtilsJNI.tileDir() + MemoryOperationUtilsX64.class.getSimpleName() + ".ini",hex.toUpperCase());
        }else {
    ...
    public static void writeIniFile(String filePath,String hex,String ten){
        if(StringUtils.equals(filePath,null)){
            IniConfigurationOperationUtils.nio_WriteByte(ModuleOperationUtilsJNI.tileDir()+MemoryOperationUtilsX64.class.getSimpleName()+".ini",hex.toUpperCase(),ten);
        }else {
     ...
     * @return 返回地址
     */
    public static long VirtualAllocEx(int hProcess, long lpBaseAddress,int dwSize,int flAllocationType,int flProtect){
     ...
     * @return 释放结果
     */
    public static boolean VirtualFreeEx(int hProcess, long lpBaseAddress){
