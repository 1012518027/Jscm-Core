 ...
 * Ps:本类操作系统注册表
 */
public class RegistryOperationUtilsX86 {
    /**
     * 打开注册表
     ...
     * @return 返回句柄
     */
    public static int openRegistryGroup(String registryGroup,boolean IsCreate){
        int rootHandle = -1;
        int rck = 0;
        ...
        if(registryGroupValue.length() > 0){
            ...
            if(RegistryGroupValueIndex != -1){
                ...
                if(IsCreate){
                    ...
                    rck = ModuleOperationUtilsJNI.MyOpenRegistryGroup(rootHandle,RegistryGroupValueIndexRight, 0,32);
                }else {
                    // 因为java的长度不够 常量要在C++下实现API
                    rck = ModuleOperationUtilsJNI.MyOpenRegistryGroup(rootHandle,RegistryGroupValueIndexRight, 1,32);
                }
                if(rck!=0){
                    return rck;
                }
    ...
    public static boolean RegistryGroupIsKeyValue(String KeyName,String ValueName){
        ...
        int openRegx = openRegistryGroup(KeyName,false);
    ...
    public static int selectRegistryGroupKeyName(String RegistryGroup, List<TagRegisTryGroupInfo> registrygroupinfos){
        ...
        if(openRegx != 0){
            int result = ModuleOperationUtilsJNI.RegQueryInfoKeyW(openRegx,lpcbMaxClassLen,lpcbMaxClassLen.length);
            if(result==0){
...
//            System.out.println("类型 ： " + type + "  字符串  " + "REG_QWORD | REG_QWORD_LITTLE_ENDIAN" + "   说明：  " + "一个 64 位数字。 | 小端格式的 64 位数字。Windows 旨在在小端计算机体系结构上运行。因此，该值在 Windows 头文件中定义为 REG_QWORD。");
    ...
    public static boolean refreshRegistryGroup(String RegistryGroup){
        ...
        if(openRegx != 0){
            int result = ModuleOperationUtilsJNI.RegFlushKey(openRegx);
            ModuleOperationUtilsJNI.RegCloseKey(openRegx);
            return result ==0;
        }
        return false;
    ...
    public static boolean writeByte(String RegistryGroup,String KeyName,byte[] writeByte,int dwTyoe){
        ...
        if(openRegx != 0){
            int flag = ModuleOperationUtilsJNI.RegSetValueExW(openRegx,KeyName,dwTyoe,writeByte,writeByte.length);
            if(flag ==0){
                ModuleOperationUtilsJNI.RegCloseKey(openRegx);
                return true;
            }else {
     ...
     * @return 返回结果
     */
    public static int deleteRegistryGroupKey(String RegistryGroup,String KeyName){
        int openRegx = openRegistryGroup(RegistryGroup,false);
        if(StringUtils.equals("",RegistryGroup)){
            ModuleOperationUtilsJNI.RegCloseKey(openRegx);
            return  -1;
     ...
     * @return 返回结果
     */
    public static int enumDeleteRegistryGroupKey(String RegistryGroup){
        int result = 0;
        List<String> keyNameList = new ArrayList<>();
        int length = selectRegistryGroupName(RegistryGroup,keyNameList);
        if(length > 0){
     ...
     * @return 返回结果
     */
    public static int deleteRegistryValueName(String RegistryGroup,String KeyName){
        int openRegx = openRegistryGroup(RegistryGroup,false);
        if(StringUtils.equals("",RegistryGroup)){
            ModuleOperationUtilsJNI.RegCloseKey(openRegx);
            return  -1;
     ...
     * @return 返回结果
     */
    public static int enumDeleteRegistryGroupValueNames(String RegistryGroup){
        int result = 0;
        List<TagRegisTryGroupInfo> preregistrations = new ArrayList<>();
        int length = selectRegistryGroupKeyName(RegistryGroup,preregistrations);
        if(length > 0){
